...
bool reader_debug_output_enabled() {
    const char *value = std::getenv("HPRLP_PRINT_DEBUG_INFO");
...
void build_model_from_arrays(const CSRMatrix *csr_A,
    ...
    if (reader_debug_output_enabled()) {
        std::cout << "problem information: nRow = " << m << ", nCol = " << n
                  << ", nnz A = " << nnz << std::endl << std::endl;
    }
