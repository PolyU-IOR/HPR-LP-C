...
HPRLP_FLOAT power_method_cusparse(HPRLP_workspace_gpu *workspace, int max_iter, HPRLP_FLOAT tol)
{
    ...
    for (int i = 1; i <= max_iter; ++i)
    {
        ...
        CUSPARSE_CHECK(hprlp_run_spmvop(
            ...
            workspace->spmv_AT->operation,
            &workspace->spmv_AT->alpha, &workspace->spmv_AT->beta,
            workspace->spmv_AT->y_cusparseDescr,
        ...
        if (i % check_every == 0)
        {
            ...
            // q = z - lambda * q
            axpby(-lambda, q, 1.0, z, q, m, workspace->stream);
