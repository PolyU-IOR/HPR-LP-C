#include "batch/batched_solver.h"
...
extern "C" HPRLP_batched_results solve_batched(const LP_info_cpu *model,
    ...
    power_ws.m = model->m;
    power_ws.n = model->n;
    allocate_memory(&power_ws, &shared_lp);
    analyze_spmv_pattern(&power_ws, &matrix_param);
    ...
    // The shared A/AT storage may use CUDA VMM.  Stream destruction can return
    // before queued work is complete, so finish all users before unmapping it.
    CUDA_CHECK(cudaStreamSynchronize(ws.stream));
