...
void create_zero_vector_device_compressible(HPRLP_FLOAT* &x, int n) {
    CUDA_CHECK(hprlp_device_malloc_compressible(&x, n * sizeof(HPRLP_FLOAT)));
    set_vector_value_device(x, n, 0.0);
}
...
void fetch_reduction_scalars(HPRLP_workspace_gpu *ws) {
    CUDA_CHECK(cudaMemcpyAsync(ws->reduction_scalars_host, ws->reduction_scalars,
                               14 * sizeof(HPRLP_FLOAT), cudaMemcpyDeviceToHost, ws->stream));
